...
# Define the API URL for fetching NHL playoff data
API_URL = "https://site.api.espn.com/apis/site/v2/sports/hockey/nhl/scoreboard?dates=20250419-20250623"
    ...
    for event in data.get('events', []):
        for competition in event.get('competitions', []):  # Iterate over each competition/game
            ...
            if status_info.get('type', {}).get('completed', False):
                ...
                # NHL periods: 1st, 2nd, 3rd, OT (overtime)
                if period_num <= 3:
                    game_segment = f"{period_num}{'st' if period_num == 1 else 'nd' if period_num == 2 else 'rd'}"
                else:
                    game_segment = "OT"  # Overtime in NHL playoffs
            ...
            game_details = {
                "sport": "NHL",  # Add sport identifier at the game level for template access
...
def merge_original_and_series(data, series):
    ...
    return {
        "sport": "NHL",  # Add sport identifier (root level for consistency)
...
if __name__ == "__main__":
    print("Fetching NHL playoff data...")

    # Define all playoff rounds and their corresponding regions
    playoff_rounds = [
        ("East 1st Round", "nhl_east_1st_round_gpt.json"),
        ("West 1st Round", "nhl_west_1st_round_gpt.json"),
        ("East 2nd Round", "nhl_east_2nd_round_gpt.json"),
        ("West 2nd Round", "nhl_west_2nd_round_gpt.json"),
        ("East Final", "nhl_east_final_gpt.json"),
        ("West Final", "nhl_west_final_gpt.json"),
        ("Stanley Cup Final", "nhl_stanley_cup_final_gpt.json")
    ]
