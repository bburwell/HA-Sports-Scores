...
def debug_print_slugs():
    response = urllib.request.urlopen(API_URL)
    data = json.loads(response.read())
    slugs = {}
    for event in data.get('events', []):
        slug = event.get('season', {}).get('slug', 'MISSING')
        slugs[slug] = slugs.get(slug, 0) + 1
    print("\n=== ALL SLUGS IN API RESPONSE ===")
...
def fetch_and_filter_data(stage_or_group):
    try:
        ...
        if stage_or_group.startswith("group-"):
            ...
            print(f"  Looking for slug: '{target_slug}'")
            filtered = [
...
def create_series_structure(data, stage_or_group):
    ...
    for event in data['events']:
        for comp in event.get('competitions', []):
            ...
            # For groups: skip if not involving defined teams
            ...
            broadcast = ", ".join(
                b.get('names', ['TBD'])[0] for b in comp.get('broadcasts', []) if b.get('names')
            )
    ...
    if stage.startswith("Group"):
        ...
        # 3rd place: best 8 of 12 advance; full logic needs all groups
        if len(sorted_teams) >= 3:
            advancing_teams.add(sorted_teams[2])
...
if __name__ == "__main__":
    ...
    # Prints all slugs found in the API response so we can verify the 3rd place slug
    debug_print_slugs()
